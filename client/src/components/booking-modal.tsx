...
export default function BookingModal({ isOpen, onClose }: BookingModalProps) {
  ...
  const processPaymentMutation = useMutation({
    mutationFn: async ({ sourceId, amount }: { sourceId: string; amount: number }) => {
  ...
  const handleStep1Submit = async (data: FormData) => {
    if (!data.readinessCheck) {
      toast({
        ...
        description: "Please confirm that you will have all necessary items ready for the audit.",
        variant: "destructive",
      });
      return;
    }
          ...
          <form onSubmit={form.handleSubmit(handleStep1Submit)} className="space-y-6">
            ...
            <div className="flex items-start space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => form.setValue("readinessCheck", checked as boolean)}
                data-testid="checkbox-readiness"
              />
              <Label htmlFor="readinessCheck" className="text-sm leading-5">
                I confirm that I will have all valuable items, receipts, and warranty papers ready for the audit appointment
              </Label>
            </div>
            ...
            >
              {createAppointmentMutation.isPending ? "Creating..." : "Proceed to Payment"}
            </Button>
          </form>
        )}
          ...
          <div className="space-y-6">
            <Card className="bg-gray-50">
              <CardContent className="p-6">
                <h4 className="font-semibold text-gray-900 mb-2" data-testid="text-service-fee">Service Fee</h4>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Professional Home Audit</span>
                  <span className="font-bold text-xl text-gray-900" data-testid="text-amount">$50.00</span>
                </div>
                <p className="text-sm text-gray-500 mt-2">One-time audit fee • Includes comprehensive documentation</p>
              </CardContent>
            </Card>
            ...
            <div className="flex space-x-4">
              ...
              <Button
                ...
                disabled={!squareReady || isProcessingPayment || processPaymentMutation.isPending}
                className="flex-1 bg-success-500 hover:bg-success-600"
                data-testid="button-complete-booking"
              >
                {isProcessingPayment || processPaymentMutation.isPending ? "Processing..." : "Complete Booking"}
              </Button>
            </div>
          </div>
        )}
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium" data-testid="text-confirmed-date">
                      {new Date(confirmedAppointment.preferredDate).toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric'
                      })}
                    ...
                    <span className="font-medium" data-testid="text-confirmed-time">
                      {confirmedAppointment.preferredTime ?
                        (confirmedAppointment.preferredTime === 'morning' ? 'Morning (9 AM - 12 PM)' :
                         ...
                         confirmedAppointment.preferredTime) : 'To be confirmed'
                      }
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium text-right max-w-xs" data-testid="text-confirmed-address">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium" data-testid="text-confirmed-amount">$50.00</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-primary-200 bg-primary-50">
              <CardContent className="p-6">
                <h5 className="font-semibold text-primary-900 mb-2 flex items-center">
                  <Badge className="bg-primary-100 text-primary-800 mr-2">Optional</Badge>
                  Add Title Protection Service
                </h5>
                <p className="text-primary-700 text-sm mb-4">
                  Monitor your property title 24/7 for unauthorized changes or fraud attempts
                </p>
                <div className="flex items-center justify-between">
